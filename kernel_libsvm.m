function [model] = kernel_libsvm(X, Y, Xtest, Ytest, kernel)
...
% [yTesthat,accTest,valsTest] = svmpredict(Ytest, [(1:size(Ktest,1))' Ktest], model);
% [yTrainhat,accTrain,valsTrain] = svmpredict(Y, [(1:size(K,1))' K], model);
% test_accuracy = 1- mean(yTesthat~=Ytest);
% train_accuracy = 1- mean(yTrainhat~=Y);

% Optionally we can look at more information from training/testing.
% infoTrain.vals = valsTrain;
% infoTrain.yhat = yTrainhat;
% infoTrain.model = model;
% infoTest.vals = valsTest;
% infoTest.yhat = yTesthat;
% infoTest.model = model;
